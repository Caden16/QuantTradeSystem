...
def save_select_result(select_result):
    select_result_collection = DATABASE.select_stock_result
    date = select_result[0]["date"]
    exist_result_cursor = get_select_result(date)
    exist_result = []
    for item in exist_result:
        exist_result.append(item)
    if exist_result is not None and len(exist_result) >= 1:
        select_result = merge_sselect_result(select_result, exist_result)
    date = select_result_collection.remove({"date": date}, {"justOne": False})
...
def merge_sselect_result(result1, result2):
    """
    合并同一天的选股结果
    :param result1:
    :param result2:
    ...
    for result_dict in result1:
        ...
        same_df = result2_df[result2_df["code"] == code]
        if len(same_df) >= 1:
            result2_para = same_df["parameter"]
            para_dict = dict(para_dict, result2_para)
            result_dict["parameter"] = para_dict
        merge_result.append(result_dict)
        result2_df = result2_df[result2_df["code"] != code]
    result2_rest = result2_df.to_dict("records")
    merge_result = dict(merge_result, result2_rest)
    return merge_result
